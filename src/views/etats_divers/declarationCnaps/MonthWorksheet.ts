...
class MonthWorksheet {
  ...
  private travailleurData: IDnsGeneratorTravailleurProps[] | null
  private employeurData: IDnsGeneratorEmployeurData | null
  private worksheet: ExcelJS.Worksheet
  private startCharCode: number
  private endCharCode: number
  private columnData: string[]
  private A1: ExcelJS.Cell
  private B1: ExcelJS.Cell
  private D1: ExcelJS.Cell
  private E1: ExcelJS.Cell
  private F1: ExcelJS.Cell
  private H1: ExcelJS.Cell
  private I1: ExcelJS.Cell
  private J1: ExcelJS.Cell
  private K1: ExcelJS.Cell
  private M1: ExcelJS.Cell
  private P1: ExcelJS.Cell
  private B2: ExcelJS.Cell
  private C2: ExcelJS.Cell
  private F2: ExcelJS.Cell
  private G2: ExcelJS.Cell
  private K2: ExcelJS.Cell
  private L2: ExcelJS.Cell
  private M2: ExcelJS.Cell
  private N2: ExcelJS.Cell
  private O2: ExcelJS.Cell
  ...
  isTravailleurDataExist = (): boolean => {
    ...
    if (this.isTravailleurDataExist()) {
      ...
      formatedData = this.travailleurData.map((item, key) => {
        ...
        return {
          ...
          prenom: item.prenom.toUpperCase(),
          num_cnaps: item.num_cnaps.toUpperCase(),
  ...
  clearAllCellValues() {
    ...
    if (formatedData) {
      for (let i = 0; i < formatedData.length; i++) {
        ...
        // row.splice(1,columnNames.length);
