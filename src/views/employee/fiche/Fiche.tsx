...
interface RenderTabProps {
  ...
  activeTab: string
  handleTabClick: (
    event: React.MouseEvent<HTMLElement, MouseEvent> | string,
  ) => void
}
...
const Fiche: React.FC = () => {
  ...
  const employees: Array<any> = useSelector(
    (state: any) => state.employeesList.list,
  )
  const location = useLocation()
  const navigate = useNavigate()

  const selectedEmployee: object = useSelector(
    (state: any) => state.bulletinDePaie.salarie,
  )
  ...
  const tabList: Array<{ key: string; label: string }> = [
